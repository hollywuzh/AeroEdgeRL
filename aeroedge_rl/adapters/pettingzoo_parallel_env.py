"""Optional PettingZoo ParallelEnv adapter for AeroEdgeRL."""
...
class GradysUAVServiceParallelEnv(ParallelEnv):
    ...
    consume simultaneous multi-agent environments.
    """

    metadata = {"name": "gradys_uav_service_v0", "render_modes": ["ansi"]}

    def __init__(self, config: Optional[UAVServiceEnvConfig] = None) -> None:
        if np is None or spaces is None or ParallelEnv is object:
            raise ImportError(
                "Install PettingZoo support with `python -m pip install -e '.[marl]'` "
                "to use GradysUAVServiceParallelEnv."
            )
