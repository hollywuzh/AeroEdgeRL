"""Optional RLlib MultiAgentEnv adapter for AeroEdgeRL."""
...
class GradysUAVServiceRLlibEnv(MultiAgentEnv):
    """RLlib multi-agent wrapper with one action per UAV."""

    def __init__(self, config: Optional[dict] = None) -> None:
        if np is None or spaces is None or MultiAgentEnv is object:
            raise ImportError(
                "Install RLlib support with `python -m pip install -e '.[rllib]'` "
                "to use GradysUAVServiceRLlibEnv."
            )
    ...
    def reset(self, *, seed=None, options=None):
        ...
        return (
            self._as_observations(observations),
            {agent: {} for agent in self.possible_agents},
        )

    def step(self, action_dict):
        result = self.core.step(
            {agent: int(action_dict.get(agent, 0)) for agent in self.possible_agents}
        )
        ...
        if self._team_reward:
            ...
            rewards = {
                agent: float(result.rewards[agent])
                for agent in self.possible_agents
            }
    ...
    def _action_mask_for(self, agent):
        mask = np.asarray(self.core.action_mask(agent), dtype=np.float32)
        if self._mask_hover_when_candidates and mask[1:].any():
            mask[0] = 0.0
        return mask
