"""Optional centralized Gymnasium adapter for AeroEdgeRL."""
...
class GradysUAVServiceGymEnv(gym.Env if gym is not None else object):
    """Centralized single-agent view over all UAV agents.

    The action is a `MultiDiscrete` vector with one task-choice action per UAV.
    This adapter is convenient for centralized PPO baselines and quick smoke
    tests. For multi-agent training, prefer the PettingZoo or RLlib adapters.
    """

    metadata = {"render_modes": ["ansi"]}

    def __init__(self, config: Optional[UAVServiceEnvConfig] = None) -> None:
        if gym is None or np is None or spaces is None:
            raise ImportError(
                "Install Gymnasium support with `python -m pip install -e '.[gym]'` "
                "to use GradysUAVServiceGymEnv."
            )

        self.core = GradysUAVServiceCoreEnv(config)
        self.action_space = spaces.MultiDiscrete(
            [self.core.action_size] * len(self.core.agents)
        )
    ...
    def step(self, action):
        ...
        return (
            self._flatten(result.observations),
            reward,
            result.terminated,
            result.truncated,
            info,
    ...
    def _flatten(self, observations):
        return np.asarray(
            flatten_agent_dict(observations, self.core.agents),
            dtype=np.float32,
        )
